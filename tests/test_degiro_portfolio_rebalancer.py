...
class TestSetupConnection(unittest.TestCase):
    ...
    def test_setup_connection_with_no_account_returns_api(
        self, MockTradingAPI, MockCredentials
    ):
        api = setup_connection(None, self.username, self.password, self.totp_secret_key)
        assert api is MockTradingAPI()

    @mock.patch("degiro_portfolio_rebalancer.Credentials")
    @mock.patch("degiro_portfolio_rebalancer.TradingAPI")
    def test_setup_connection_with_account_returns_api(
        self, MockTradingAPI, MockCredentials
    ):
        api = setup_connection(
            1234567, self.username, self.password, self.totp_secret_key
        )
        assert api is MockTradingAPI()
    ...
    def test_validate_credentials_fetches_account_number(self, MockCredentials):
        with mock.patch("degiro_portfolio_rebalancer.TradingAPI") as MockTradingAPI:
            ...
            client_details.__getitem__.side_effect = lambda name: {
                "intAccount": 1234567
            }
