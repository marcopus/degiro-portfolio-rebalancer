...
def read_configuration_from_file(account_name: str, config_dirpath: str="config") -> dict:
    """
...
def rebalance(config: dict):
    ...
    reference_date_from = dp.parse(config.get('reference_date_from'), settings={'DATE_ORDER': 'YMD'})
    reference_date_to = dp.parse(config.get('reference_date_to'), settings={'DATE_ORDER': 'YMD'})
...
if __name__ == '__main__':
    ...
    # SETUP CONFIG DICT
    if len(sys.argv) < 2:
        print("Missing account name", "Usage: python degiro_portfolio_rebalancer.py <account_name>", sep="\n")
        sys.exit(64)
